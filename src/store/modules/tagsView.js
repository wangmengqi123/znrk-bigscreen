...
const mutations = {
  ADD_VISITED_VIEW: (state, view) => {
    ...
    // Store the i18n key or title as-is, translation will happen in components
  ...
  DEL_LEFT_CACHED_VIEWS: (state, view) => {
    ...
    const leftViews = state.visitedViews.slice(0, index);
    const leftNames = leftViews.map((i) => i.name);
    state.cachedViews = state.cachedViews.filter(
      (name) => !leftNames.includes(name)
    );
  },
  DEL_RIGHT_CACHED_VIEWS: (state, view) => {
    ...
    const rightViews = state.visitedViews.slice(index + 1);
    const rightNames = rightViews.map((i) => i.name);
    state.cachedViews = state.cachedViews.filter(
      (name) => !rightNames.includes(name)
    );
  },
